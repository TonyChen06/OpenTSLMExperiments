...
"""
Evaluate OpenTSLMFlamingo on TSQA dataset .
...
def run_evaluation(
    ...
    """Run evaluation on the dataset with optional signal tracking."""
...
def main():
    ...
    if args.use_noise:
        ...
        TSQADataset.set_noise_mode(use_noise=True, noise_type=args.noise_type, noise_level=args.noise_level, noise_seed=args.noise_seed)
    else:
        TSQADataset.set_noise_mode(use_noise=False)

    # Configure signal blocking
    if args.use_block:
        print(f"[BLOCK MODE] total={args.block_total_sec}s, avg={args.block_avg_sec}s, std={args.block_std_sec}s, seed={args.block_seed}")
        TSQADataset.set_block_mode(use_block=True, block_total_sec=args.block_total_sec, block_avg_sec=args.block_avg_sec, block_std_sec=args.block_std_sec, block_seed=args.block_seed)
