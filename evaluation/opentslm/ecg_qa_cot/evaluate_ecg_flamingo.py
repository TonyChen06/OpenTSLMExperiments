...
"""
Evaluate OpenTSLMFlamingo on ECG-QA CoT dataset .
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate OpenTSLMFlamingo on ECG-QA")
    ...
    if args.use_noise:
        ...
        print(f"[NOISE MODE] ECG signals will be blended with {args.noise_type} noise (seed={args.noise_seed}{level_msg})")
        ECGQACoTQADataset.set_noise_mode(use_noise=True, noise_type=args.noise_type, noise_level=args.noise_level, noise_seed=args.noise_seed)
    else:
    ...
    if args.use_block:
        ...
        ECGQACoTQADataset.set_block_mode(use_block=True, block_total_sec=args.block_total_sec, block_avg_sec=args.block_avg_sec, block_std_sec=args.block_std_sec, block_seed=args.block_seed)

    # Load model
    model = load_model(args.checkpoint, device, args.llm_id)

    # Load dataset
    print("Loading ECG-QA CoT dataset (test split)...")
    dataset = ECGQACoTQADataset(
        split="test",
